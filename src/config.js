module.exports = {
   defaultTitle: 'Matthew Griffin - Portfolio',
   logo: 'https://mgrif.xyz/assets/avatar.png',
   author: 'Matthew Griffin',
   url: 'https://mgrif.xyz',
   email: 'matt@mgrif.xyz',
   legalName: 'Matthew Griffin',
   defaultDescription: `Heya! I'm Matthew Griffin and I'm a developer and digital creator.`,
   socials: [
      {
         name: 'GitHub',
         url: 'https://github.com/xMdb',
      },
      {
         name: 'Twitter',
         url: 'https://twitter.com/xMdbMatt',
         handle: '@xMdbMatt',
      },
      {
         name: 'Instagram',
         url: 'https://instagram.com/xMdbMatt',
      },
      {
         name: 'YouTube',
         url: 'https://youtube.com/xMdbMatt',
      },
   ],
   nav: [
      {
         name: 'About',
         url: '/#about',
      },
      {
         name: 'Experience',
         url: '/#jobs',
      },
      {
         name: 'Work',
         url: '/#projects',
      },
      {
         name: 'Contact',
         url: '/#contact',
      },
   ],
   googleAnalyticsID: 'G-KDFQFND2W5',
   keywords:
      'developer portfolio, developer, portfolio, matthew griffin, matthew griffin xMdb, xMdb, xMdbMatt, xMdb portfolio, xMdbMatt developer, matthew griffin developer, matt xmdb developer, xmbd developer, xmbd, gatsby portfolio, gatsbyjs portfolio, web development, web developer, webdev, matthew griffin portfolio, built with gatsby, matthew griffin website, xmdb is a dev, mgrif, mgrif xyz, matt g, matthew g, web design, web development designer, matthew designer, matthew griffin designer, figma designer, front end development, back end development, development, reactjs developer, react developer, built with react, react portfolio website, react portfolio, front end developer, back end developer, xmbd matt, xmbdmatt, matty griffin, matty g',
   colours: {
      green: '#64ffda',
      navy: '#0a192f',
      darkNavy: '#020c1b',
   },
   foundingDate: '2019',
};
